            ...
            <div class="team-member-wrap">
                ...
                <div class="m-t-10 text-align-start text-align-md-center">
                    @if ($instructors->nextPageUrl())
                        <div class="instructor-pagination text-align-center text-align-lg-center m-t-sm-10">
                ...
                @if (count($best_teacher) > 0)
                    @foreach ($best_teacher as $instructor)
                        ...
                        <div class="col-xl-3 col-lg-4 col-sm-6" data-aos="fade-up"
                            data-aos-delay="{{ 200 * $loop->iteration }}">
                            <div class="team-member-item">
                                <div class="member-img">
                                    <a
                                        href="{{ $instructor->slug ? route('instructor.details', $instructor->slug) : '#' }}"><img
                                            src="{{ getFileLink('128x128', $instructor->user->images) }}"
                                            alt="{{ $instructor->user->name }}"></a>
                                </div>
                                <div class="member-content">
                                    <h5>
                                        <a
                                            href="{{ $instructor->slug ? route('instructor.details', $instructor->slug) : '#' }}">{{ $instructor->user->name }}</a>
                                    </h5>
    ...
    <section class="best-teacher-section p-t-80 p-b-50 p-t-sm-50 p-b-sm-30">
        <div class="container container-1278">
            ...
            <div class="row team-member-items-v2">
                @if (count($best_teacher) > 0)
                    @foreach ($best_teacher as $key => $teacher)
                        @include('frontend.instructor.component.best_teacher')
                    @endforeach
                @else
                    @include('frontend.not_found', $data = ['title' => 'best teacher'])
                @endif
            ...
            <div class="m-t-10 text-align-start text-align-md-center">
                @if ($best_teacher->nextPageUrl())
                    <div class="instructor-pagination text-align-center text-align-lg-center m-t-sm-10">
                        <a data-page="{{ $best_teacher->currentPage() }}" href="javascript:void(0)"
                            class="template-btn load_more">{{ __('see_more') }}
                            <i class="fas fa-long-arrow-right"></i></a>
                        @include('components.frontend_loading_btn', [
                            'class' => 'template-btn see-more',
    ...
    <section class="become-instructor-section color-bg-off-white p-t-60 p-b-70 p-t-sm-40 p-b-sm-50 position-relative">
        <div class="bg-shape">
            <svg width="482" height="424" viewBox="0 0 482 424" fill="none" xmlns="http://www.w3.org/2000/svg">
                <mask id="mask0_1932_5807" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="482"
                    height="424">
            ...
            <div class="row align-items-center">
                ...
                <div class="col-lg-6 col-md-5" data-aos="fade-up" data-aos-delay="400">
                    <div class="become-instructor-image">
                        <img src="{{ getFileLink('391x541', setting('become_instructor_image')) }}"
...
@push('js')
    <script>
        $(document).ready(function() {
            $(document).on('click', '.instructor-section .load_more', function() {
                instructor('load_more');
            });
            $(document).on('click', '.best-teacher-section .load_more', function() {
                var that = $(this);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        let selector = $('.team-member-items-v2');
